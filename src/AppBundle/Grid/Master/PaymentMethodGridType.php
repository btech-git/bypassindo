...
class PaymentMethodGridType extends DataGridType
{
    public function buildWidgets(WidgetsBuilder $builder, array $options)
    {
        $builder->searchWidget()
            ->addGroup('paymentMethod')
                ->setEntityName(PaymentMethod::class)
                ->addField('name')
    ...
    public function buildData(DataBuilder $builder, ObjectRepository $repository, array $options)
    {
        ...
        $builder->processPage($repository->count($criteria['paymentMethod'], $associations), function($offset, $size) use ($criteria) {
            $criteria['paymentMethod']->setMaxResults($size);
            $criteria['paymentMethod']->setFirstResult($offset);
        });
        
        $objects = $repository->match($criteria['paymentMethod'], $associations);
